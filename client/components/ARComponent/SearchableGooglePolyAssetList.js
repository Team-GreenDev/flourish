  ...
  renderCurrentResults() {
    ...
    for (var i = 0; i < this.state.currentResults.length; i += 2) {
      if (i == this.state.currentResults.length - 1) {
        results.push(
          <GooglePolyAsset onPress={this.props.onAssetPress} asset={this.state.currentResults[i]} key={i} />
        );
        break;
      }
      ...
      results.push(
        <View style={{flexDirection:"row"}} key={"row" + i}>
          <GooglePolyAsset onPress={this.props.onAssetPress} asset={this.state.currentResults[i]} key={i} />
          <GooglePolyAsset onPress={this.props.onAssetPress} asset={this.state.currentResults[i+1]} key={i+1} />
        </View>
      );
    }
...
const styles = StyleSheet.create({
  searchContainer: {
    ...
    borderWidth: 1,  borderRadius: 10, borderColor: "#DDDDDD",
  },
  searchTextInput: { flex: 1,  height:40, },
  noResultsText: { fontSize: 18, fontStyle: "italic", paddingTop: 50, }
});